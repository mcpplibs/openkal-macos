...
// records as genuinely borrowed. It is NOT in the stub: the stub lists names
...
//
// Measured 2026-08-23, cross-linking for this system from Linux with the
...
// NO INCLUDES. This package's other sources take none either, and the reason
...
// WEAK, because a program with no exceptions has no `__eh_frame`, and a
...
// libunwind. The order and widths are the contract; a disagreement here
...
extern "C" bool _dyld_find_unwind_sections(void*, dyld_unwind_sections* info) {
    ...
    // The address is ignored, and that is correct rather than lazy: there is
    ...
    // False when there is nothing to find, not "true with zero length".
